...
const TodoParams = Type.Object({
	...
	subtask_id: Type.Optional(Type.String({ description: "Subtask ID (for subtask operations)" })),
	subtask_title: Type.Optional(Type.String({ description: "Subtask title (for add/update-subtask)" })),
	subtask_status: Type.Optional(Type.String({ description: "Subtask status (for update-subtask)" })),
	subtask_titles: Type.Optional(
		Type.Array(Type.String({ description: "Subtasks titles to create" })),
	),
	subtask_updates: Type.Optional(
		Type.Array(
			Type.Object({
				subtask_id: Type.String({ description: "Subtask ID" }),
				status: Type.Optional(
					StringEnum(["open", "in-progress", "done"] as const),
				),
				title: Type.Optional(Type.String({ description: "Updated subtask title" })),
			}),
		),
	),
	priority: Type.Optional(Type.String({ description: "Todo priority: low, medium, high" })),
	due_date: Type.Optional(Type.String({ description: "Due date in ISO format" })),
});
...
function parseFrontMatter(text: string, idFallback: string): TodoFrontMatter {
	...
	try {
		...
		if (Array.isArray(parsed.subtasks)) {
			data.subtasks = parsed.subtasks.filter((st): st is Subtask => 
				typeof st === "object" && st !== null &&
				typeof st.id === "string" &&
				typeof st.title === "string" &&
				(typeof st.status === "string" && ["open", "in-progress", "done"].includes(st.status))
			);
		}
		if (typeof parsed.priority === "string" && ["low", "medium", "high"].includes(parsed.priority)) {
			data.priority = parsed.priority as "low" | "medium" | "high";
		}
...
function parseTodoContent(content: string, idFallback: string): TodoRecord {
	...
	// Spread parsed frontmatter so all fields (including subtasks, priority,
	// due_date, and any future fields) survive the read->write round-trip.
	return {
		...parsed,
		id: idFallback,
	...
	for (let attempt = 0; attempt < MAX_RETRIES; attempt += 1) {
		try {
			...
			if (error?.code !== "EEXIST") {
				return { error: "Error accessing todo. Try again." };
			}
			// Another call has the lock. Retry with backoff — most operations
			// complete in under 50ms, so a quick retry almost always succeeds.
			if (attempt < MAX_RETRIES - 1) {
				await sleep(200 * (attempt + 1));
				continue;
			}
			// Last attempt failed. Check if the lock is stale (e.g. crashed session).
			const now = Date.now();
			const stats = await fs.stat(lockPath).catch(() => null);
			const lockAge = stats ? now - stats.mtimeMs : LOCK_TTL_MS + 1;
			if (lockAge <= LOCK_TTL_MS) {
				return { error: "That task is busy syncing changes. Try again in a moment." };
			}
			// Stale lock — offer to steal in interactive mode.
			if (!ctx.hasUI) {
				return { error: `Todo lock is stale. Restart pi or run in interactive mode to steal it.` };
			}
			const ok = await ctx.ui.confirm(
				"Todo locked",
				`Todo appears locked (stale). Steal the lock?`,
			);
			if (!ok) {
				return { error: `That task remains locked. Try again later.` };
			}
...
function serializeTodoForAgent(todo: TodoRecord): string {
	const tree = renderSubtasksTree(todo);
	const payload = { ...todo, id: formatTodoId(todo.id), subtask_tree: tree };
...
function serializeTodoListForAgent(todos: TodoFrontMatter[]): string {
	...
	const mapTodo = (todo: TodoFrontMatter) => {
		const result: Record<string, unknown> = { ...todo, id: formatTodoId(todo.id) };
		const subtasks = todo.subtasks ?? [];
		if (subtasks.length > 0) {
			result.subtask_summary = {
				total: subtasks.length,
				open: subtasks.filter((s) => s.status === "open").length,
				in_progress: subtasks.filter((s) => s.status === "in-progress").length,
...
function renderTodoDetail(theme: Theme, todo: TodoRecord, expanded: boolean): string {
	...
	if (todo.subtasks && todo.subtasks.length > 0) {
		...
		lines.push(
			"",
			theme.fg("muted", `Subtasks (${todo.subtasks.length}):`),
...
function renderSubtasksTree(todo: TodoRecord): string {
	const parentStatus = getTodoStatus(todo);
	const parentIcon = parentStatus === "done" || parentStatus === "closed" ? "✅" : parentStatus === "in-progress" ? "🔶" : "⬜";
	const parentLine = `${parentIcon} ${formatTodoId(todo.id)} "${todo.title || "(untitled)"}" [${parentStatus}]`;

	const subtasks = todo.subtasks;
	if (!subtasks || subtasks.length === 0) return parentLine;

	const sorted = sortSubtasks(subtasks);
	const lines: string[] = [parentLine];
	for (let i = 0; i < sorted.length; i++) {
		const st = sorted[i]!;
		const isLast = i === sorted.length - 1;
		const branch = isLast ? "└── " : "├── ";
		const icon = st.status === "done" ? "✅" : st.status === "in-progress" ? "🔶" : "⬜";
		const idPart = `SUB-${st.id}`;
		let line = `${branch}${icon} ${idPart} ${st.title || "(untitled)"} [${st.status}]`;
		if (st.completed_at && st.status === "done") {
			line += ` • ${st.completed_at}`;
		}
		lines.push(line);
	}
	return lines.join("\n");
...
function generateSubtaskId(existing: Subtask[]): string {
	for (let attempt = 0; attempt < 10; attempt += 1) {
		const id = crypto.randomBytes(2).toString("hex");
		if (!existing.some((st) => st.id === id)) return id;
	}
	throw new Error("Failed to generate unique subtask id");
}

function validateSubtaskId(id: string): { id: string } | { error: string } {
	const trimmed = id.trim();
	if (!trimmed || !/^[a-f0-9]{4}$/i.test(trimmed)) {
		return { error: "Invalid subtask id. Expected 4-character hex string." };
	}
	return { id: trimmed.toLowerCase() };
}

function formatSubtaskId(id: string): string {
	return `SUB-${id}`;
}

function isSubtaskComplete(status: string): boolean {
	return status === "done";
}

function sortSubtasks(subtasks: Subtask[]): Subtask[] {
	return [...subtasks].sort((a, b) => {
		const aComplete = isSubtaskComplete(a.status);
		const bComplete = isSubtaskComplete(b.status);
...
async function addSubtask(
	...
	const validated = validateTodoId(todoId);
	if ("error" in validated) {
		return { error: validated.error };
	}
	const normalizedId = validated.id;
	const filePath = getTodoPath(todosDir, normalizedId);
	if (!existsSync(filePath)) {
		return { error: `Todo ${displayTodoId(todoId)} not found` };
	}

	const result = await withTodoLock(todosDir, normalizedId, ctx, async () => {
		const existing = await ensureTodoExists(filePath, normalizedId);
		if (!existing) return { error: `Todo ${displayTodoId(todoId)} not found` } as const;

		const trimmedTitle = title.trim();
		if (!trimmedTitle) {
			return { error: "Subtask title must not be empty" } as const;
		}

		if (!existing.subtasks) existing.subtasks = [];
		const subtask: Subtask = {
			id: generateSubtaskId(existing.subtasks),
			title: trimmedTitle,
			status: "open",
			created_at: new Date().toISOString(),
		};
		existing.subtasks.push(subtask);
		existing.subtasks = sortSubtasks(existing.subtasks);
...
async function addSubtasks(
	...
	const result = await withTodoLock(todosDir, normalizedId, ctx, async () => {
		...
		for (let i = 0; i < titles.length; i += 1) {
			const trimmedTitle = titles[i].trim();
			if (!trimmedTitle) {
				return { error: `Subtask title at index ${i} is empty` } as const;
			}
			const subtask: Subtask = {
				id: generateSubtaskId(existing.subtasks),
				title: trimmedTitle,
...
async function updateSubtask(
	...
	todoId: string,
	subtaskId: string,
	updates: Partial<Pick<Subtask, "title" | "status">>,
	ctx: ExtensionContext,
): Promise<TodoRecord | { error: string }> {
	const validatedTodo = validateTodoId(todoId);
	if ("error" in validatedTodo) {
		return { error: validatedTodo.error };
	}
	const validatedSubtask = validateSubtaskId(subtaskId);
	if ("error" in validatedSubtask) {
		return { error: validatedSubtask.error };
	}
	
	const normalizedTodoId = validatedTodo.id;
	const normalizedSubtaskId = validatedSubtask.id;
	const filePath = getTodoPath(todosDir, normalizedTodoId);
	if (!existsSync(filePath)) {
		return { error: `Todo ${displayTodoId(todoId)} not found` };
	}

	const result = await withTodoLock(todosDir, normalizedTodoId, ctx, async () => {
		const existing = await ensureTodoExists(filePath, normalizedTodoId);
		if (!existing) return { error: `Todo ${displayTodoId(todoId)} not found` } as const;
		if (!existing.subtasks) {
			return { error: `Todo ${displayTodoId(todoId)} has no subtasks` } as const;
		}
		
		const subtaskIndex = existing.subtasks.findIndex(st => st.id === normalizedSubtaskId);
		if (subtaskIndex === -1) {
			return { error: `Subtask ${formatSubtaskId(normalizedSubtaskId)} not found` } as const;
		}
		
		const subtask = existing.subtasks[subtaskIndex];
		...
		if (updates.status !== undefined) {
			...
			if (updates.status === "done" && oldStatus !== "done") {
				...
				// Clear stale completion marker when reopening / moving away from done.
				subtask.completed_at = undefined;
			}
		}
		
		existing.subtasks[subtaskIndex] = subtask;
		existing.subtasks = sortSubtasks(existing.subtasks);
...
async function completeSubtask(
	...
	return updateSubtask(todosDir, todoId, subtaskId, { status: "done" }, ctx);
}
...
async function deleteSubtask(
	...
	const result = await withTodoLock(todosDir, normalizedTodoId, ctx, async () => {
		...
		existing.subtasks.splice(subtaskIndex, 1);
		await writeTodoFile(filePath, existing);
		return existing;
	});
...
async function batchUpdateSubtasks(
	todosDir: string,
	todoId: string,
	updates: { subtask_id: string; status?: Subtask["status"]; title?: string }[],
	ctx: ExtensionContext,
): Promise<TodoRecord | { error: string }> {
	const validated = validateTodoId(todoId);
	if ("error" in validated) {
	...
	for (let i = 0; i < updates.length; i += 1) {
		...
		const idCheck = validateSubtaskId(entry.subtask_id);
		if ("error" in idCheck) {
			return { error: `Subtask id at index ${i}: ${idCheck.error}` };
		}
		if (entry.status !== undefined && !["open", "in-progress", "done"].includes(entry.status)) {
			return { error: `Invalid status at index ${i}: "${entry.status}"` };
		}
	}
	...
	const result = await withTodoLock(todosDir, normalizedId, ctx, async () => {
		...
		for (const entry of updates) {
			const normalizedSubtaskId = validateSubtaskId(entry.subtask_id);
			if ("error" in normalizedSubtaskId) continue; // already validated above
			const idx = existing.subtasks.findIndex(st => st.id === normalizedSubtaskId.id);
			if (idx === -1) {
				return { error: `Subtask ${formatSubtaskId(entry.subtask_id)} not found` } as const;
			}
			const st = existing.subtasks[idx];

			if (entry.title !== undefined) {
				const trimmed = entry.title.trim();
				if (!trimmed) {
					return { error: `Subtask title at id ${entry.subtask_id} is empty` } as const;
				}
				st.title = trimmed;
			}
			...
			if (entry.status !== undefined) {
				...
				if (entry.status === "done" && oldStatus !== "done") {
					st.completed_at = new Date().toISOString();
				} else if (entry.status !== "done") {
					st.completed_at = undefined;
				}
			}

			existing.subtasks[idx] = st;
		}
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		label: "Todo",
		description:
			`Manage file-based todos in ${todosDirLabel} (list, list-all, get, create, update, append, delete, claim, release, add-subtask, add-subtasks, update-subtask, complete-subtask, delete-subtask, batch-update-subtasks). ` +
			"Title is the short summary; body is long-form markdown notes (update replaces, append adds). " +
			"Subtasks break a todo into smaller steps (ids shown as SUB-<hex>; status: open, in-progress, done). " +
			"Todo ids are shown as TODO-<hex>; id parameters accept TODO-<hex> or the raw hex filename. " +
			"Claim tasks before working on them to avoid conflicts, and close them when complete.",
		promptSnippet: "Create TODOs with subtasks to track multi-step work",
		promptGuidelines: [
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			switch (action) {
				...
				case "add-subtask": {
					if (!params.id || !params.subtask_title) {
						return {
							content: [{ type: "text", text: "Error: id and subtask_title required" }],
							details: { action: "add-subtask", error: "id and subtask_title required" },
						};
					}
					const result = await addSubtask(todosDir, params.id, params.subtask_title, ctx);
					if (typeof result === "object" && "error" in result) {
						return {
							content: [{ type: "text", text: result.error }],
							details: { action: "add-subtask", error: result.error },
						};
					}
					return {
						content: [{ type: "text", text: serializeTodoForAgent(result as TodoRecord) }],
						details: { action: "add-subtask", todo: result as TodoRecord },
					};
				}

				case "update-subtask": {
					if (!params.id || !params.subtask_id) {
						return {
							content: [{ type: "text", text: "Error: id and subtask_id required" }],
							details: { action: "update-subtask", error: "id and subtask_id required" },
						};
					}
					const updates: Partial<Pick<Subtask, "title" | "status">> = {};
					if (params.subtask_title !== undefined) updates.title = params.subtask_title;
					if (params.subtask_status !== undefined) {
						if (!["open", "in-progress", "done"].includes(params.subtask_status)) {
							return {
								content: [{ type: "text", text: "Error: subtask_status must be 'open', 'in-progress', or 'done'" }],
								details: { action: "update-subtask", error: "Invalid subtask_status" },
							};
						}
						updates.status = params.subtask_status as Subtask["status"];
					}
					const result = await updateSubtask(todosDir, params.id, params.subtask_id, updates, ctx);
					if (typeof result === "object" && "error" in result) {
						return {
							content: [{ type: "text", text: result.error }],
							details: { action: "update-subtask", error: result.error },
						};
					}
					return {
						content: [{ type: "text", text: serializeTodoForAgent(result as TodoRecord) }],
						details: { action: "update-subtask", todo: result as TodoRecord },
					};
				}
				...
				case "complete-subtask": {
					if (!params.id || !params.subtask_id) {
						return {
							content: [{ type: "text", text: "Error: id and subtask_id required" }],
							details: { action: "complete-subtask", error: "id and subtask_id required" },
						};
					}
					const result = await completeSubtask(todosDir, params.id, params.subtask_id, ctx);
					if (typeof result === "object" && "error" in result) {
						return {
							content: [{ type: "text", text: result.error }],
							details: { action: "complete-subtask", error: result.error },
						};
					}
					return {
						content: [{ type: "text", text: serializeTodoForAgent(result as TodoRecord) }],
						details: {
							...
							completed_subtask: (result as TodoRecord).subtasks?.find(
								(st) => st.id === params.subtask_id.trim().toLowerCase(),
							),
						},
					};
				}
				...
				case "delete-subtask": {
					if (!params.id || !params.subtask_id) {
						return {
							content: [{ type: "text", text: "Error: id and subtask_id required" }],
							details: { action: "delete-subtask", error: "id and subtask_id required" },
						};
					}
					const result = await deleteSubtask(todosDir, params.id, params.subtask_id, ctx);
					if (typeof result === "object" && "error" in result) {
						return {
							content: [{ type: "text", text: result.error }],
							details: { action: "delete-subtask", error: result.error },
						};
					}
					return {
						content: [{ type: "text", text: serializeTodoForAgent(result as TodoRecord) }],
						details: { action: "delete-subtask", todo: result as TodoRecord },
					};
				}

				case "add-subtasks": {
					if (!params.id || !params.subtask_titles || !Array.isArray(params.subtask_titles) || params.subtask_titles.length === 0) {
						return {
							content: [{ type: "text", text: "Error: id and subtask_titles (non-empty array) required" }],
							details: { action: "add-subtasks", error: "id and subtask_titles required" },
						};
					}
					const result = await addSubtasks(todosDir, params.id, params.subtask_titles, ctx);
					if (typeof result === "object" && "error" in result) {
						return {
							content: [{ type: "text", text: result.error }],
							details: { action: "add-subtasks", error: result.error },
						};
					}
					return {
						content: [{ type: "text", text: serializeTodoForAgent(result as TodoRecord) }],
						details: { action: "add-subtasks", todo: result as TodoRecord },
					};
				}

				case "batch-update-subtasks": {
					if (!params.id || !params.subtask_updates || !Array.isArray(params.subtask_updates) || params.subtask_updates.length === 0) {
						return {
							content: [{ type: "text", text: "Error: id and subtask_updates (non-empty array) required" }],
							details: { action: "batch-update-subtasks", error: "id and subtask_updates required" },
						};
					}
					const typedUpdates = params.subtask_updates.map((u: Record<string, unknown>) => ({
						subtask_id: String(u.subtask_id ?? ""),
						status: u.status as Subtask["status"] | undefined,
						title: typeof u.title === "string" ? u.title : undefined,
					}));
					const result = await batchUpdateSubtasks(todosDir, params.id, typedUpdates, ctx);
					if (typeof result === "object" && "error" in result) {
						return {
							content: [{ type: "text", text: result.error }],
							details: { action: "batch-update-subtasks", error: result.error },
						};
					}
					return {
						content: [{ type: "text", text: serializeTodoForAgent(result as TodoRecord) }],
						details: { action: "batch-update-subtasks", todo: result as TodoRecord },
					};
				}
			}
		},
	...
	pi.registerCommand("todos", {
		...
		handler: async (args, ctx) => {
			...
			// Live watch dashboard — auto-refreshes on file changes
			if (searchTerm.startsWith("watch")) {
				let watcher: ReturnType<typeof watch> | null = null;
				let cachedTodos = todos;
				let cachedRecords = new Map<string, TodoRecord>();
				let exitFlag = false;
				let frozenDone = false;

				const refreshCache = async () => {
					try {
						cachedTodos = await listTodos(todosDir);
						cachedRecords.clear();
						for (const todo of cachedTodos) {
							const fp = getTodoPath(todosDir, todo.id);
							const rec = await ensureTodoExists(fp, todo.id);
							if (rec) cachedRecords.set(todo.id, rec);
						}
					} catch { /* ignore rapid-change race */ }
				};

				// Pre-load subtask data so trees render on first frame
				await refreshCache();

				// If nothing to watch, bail immediately
				{
					const { assignedTodos, openTodos } = splitTodosByAssignment(cachedTodos);
					if (assignedTodos.length === 0 && openTodos.length === 0) {
						ctx.ui.notify("No active todos to watch.", "info");
						return;
					}
				}

				await ctx.ui.custom<void>((tui, theme, keybindings, done) => {
					watcher = watch(todosDir, async (_event, filename) => {
						if (filename && !filename.endsWith(".md")) return;
						if (exitFlag || frozenDone) return;
						await refreshCache();
						const { assignedTodos, openTodos } = splitTodosByAssignment(cachedTodos);
						if (assignedTodos.length === 0 && openTodos.length === 0) {
							frozenDone = true;
							watcher?.close();
							ctx.ui.notify("✅ All todos completed — dashboard frozen. Esc to close.", "success");
						}
						if (!exitFlag) tui.requestRender();
					});
					...
					return {
						...
						render(width: number) {
							const { assignedTodos, openTodos } = splitTodosByAssignment(cachedTodos);
							const display = [...assignedTodos, ...openTodos];
							const lines: string[] = [];

							if (frozenDone) {
								// Frozen final state — no blinking, no watcher, no resources
								lines.push(theme.fg("success", "✅ All todos completed"));
								lines.push(theme.fg("borderMuted", "─".repeat(Math.min(width, 60))));
								lines.push("");
								lines.push(theme.fg("muted", "  All tasks are done. Everything completed!"));
								lines.push("");
								lines.push(theme.fg("dim", "Esc to close · frozen (no background processing)"));
								return lines;
							}

							// Header with LIVE indicator
							lines.push(theme.fg("success", "● LIVE") + theme.fg("muted", `  ${display.length} active todos · auto-refresh`));
							lines.push(theme.fg("borderMuted", "─".repeat(Math.min(width, 60))));
							lines.push("");

							if (display.length === 0) {
								lines.push(theme.fg("muted", "  No active todos."));
							} else {
								for (const todo of display) {
									const rec = cachedRecords.get(todo.id);
									if (rec && rec.subtasks && rec.subtasks.length > 0) {
										const tree = renderSubtasksTree(rec);
										tree.split("\n").forEach((tl) => { if (!exitFlag) lines.push(tl); });
									} else {
										const icon = isTodoClosed(getTodoStatus(todo)) ? "✅" : todo.assigned_to_session ? theme.fg("success", "🔶") : "⬜";
										const tagText = todo.tags.length ? theme.fg("dim", ` [${todo.tags.join(", ")}]`) : "";
										lines.push(`${icon} ${theme.fg("accent", formatTodoId(todo.id))} ${theme.fg("text", todo.title || "(untitled)")}${tagText}`);
									}
									lines.push("");
								}
							}

							lines.push(theme.fg("dim", "Esc to close · auto-refreshes on file changes"));
							return lines;
						},
					};
				});
